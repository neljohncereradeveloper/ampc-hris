...
@Injectable()
export class RestoreLeaveYearConfigurationUseCase {
  constructor(
  ...
  async execute(id: number, requestInfo?: RequestInfo): Promise<boolean> {
    return this.transactionHelper.executeTransaction(
      LEAVE_YEAR_CONFIGURATION_ACTIONS.RESTORE,
      async (manager) => {
        ...
        if (!success) {
          throw new LeaveYearConfigurationBusinessException(
            'Leave year configuration restore failed',
            HTTP_STATUS.INTERNAL_SERVER_ERROR,
          );
        }

        const log = ActivityLog.create({
          action: LEAVE_YEAR_CONFIGURATION_ACTIONS.RESTORE,
