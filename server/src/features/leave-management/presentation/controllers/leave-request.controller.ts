...
export class LeaveRequestController {
    ...
    @ApiResponse({ status: 200, description: 'Leave requests loaded successfully' })
    @ApiResponse({ status: 404, description: 'Leave request not found' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiBearerAuth('JWT-auth')
    async getPaginatedLeaveRequestsByEmployee(
        ...
        return this.getPaginatedLeaveRequestByEmployeeUseCase.execute(employee_id, term ?? '', page, limit, is_archived);
    }
    ...
    @ApiResponse({ status: 200, description: 'Pending leave requests loaded successfully' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiBearerAuth('JWT-auth')
    async getPaginatedPendingLeaveRequests(
        ...
        const page = Number(queryDto.page) || 1;
        const limit = Number(queryDto.limit) || 10;
        return this.getPaginatedPendingLeaveRequestsUseCase.execute(term, page, limit);
    }
    ...
    async getPaginatedLeaveRequests(
        ...
        const limit = Number(queryDto.limit) || 10;
        const is_archived = queryDto.is_archived === 'true';
        return this.getPaginatedLeaveRequestUseCase.execute(term, page, limit, is_archived);
    }
}
