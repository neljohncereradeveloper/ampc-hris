...
export class EducationCourseRepositoryImpl implements EducationCourseRepository<EntityManager> {
  async create(
    ...
    const query = `
    INSERT INTO ${TEST_DATABASE_MODELS.EDUCATION_COURSES} (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${TEST_DATABASE_MODELS.EDUCATION_COURSES} }
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.EDUCATION_COURSES} }
      WHERE id = $1
    `;
  ...
  private entityToModel(entity: Record<string, unknown>): EducationCourse {
    return EducationCourse.fromPersistence(entity);
  }
}
