...
import { ReligionRepository } from '@/features/test/domain/repositories';
import { Religion } from '@/features/test/domain/models';
...
async create(religion: Religion, manager: EntityManager): Promise<Religion> {
  const query = `
    INSERT INTO ${ TEST_DATABASE_MODELS.RELIGIONS } (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${ TEST_DATABASE_MODELS.RELIGIONS } }
  ...
  async findById(id: number, manager: EntityManager): Promise<Religion | null> {
    const query = `
      SELECT *
      FROM ${ TEST_DATABASE_MODELS.RELIGIONS } }
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const religions = dataResult.map((row: Record<string, unknown>) =>
   ...
   */
  async combobox(manager: EntityManager): Promise<Religion[]> {
    const query = `
      SELECT *
      FROM ${ TEST_DATABASE_MODELS.RELIGIONS } }
      WHERE deleted_at IS NULL
  ...
  private entityToModel(entity: Record<string, unknown>): Religion {
    return Religion.fromPersistence(entity);
  }
}
