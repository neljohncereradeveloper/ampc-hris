...
export class EducationCourseLevelRepositoryImpl implements EducationCourseLevelRepository<EntityManager> {
  async create(
    ...
    const query = `
    INSERT INTO ${TEST_DATABASE_MODELS.EDUCATION_COURSE_LEVELS} (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${TEST_DATABASE_MODELS.EDUCATION_COURSE_LEVELS} }
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.EDUCATION_COURSE_LEVELS} }
      WHERE id = $1
    `;
  ...
  private entityToModel(entity: Record<string, unknown>): EducationCourseLevel {
    return EducationCourseLevel.fromPersistence(entity);
  }
}
