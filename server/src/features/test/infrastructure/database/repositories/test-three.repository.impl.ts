...
@Injectable()
export class TestThreeRepositoryImpl implements TestThreeRepository<EntityManager> {
  async create(
    test_three: TestThree,
    manager: EntityManager,
  ): Promise<TestThree> {
    const query = `
    INSERT INTO ${TEST_DATABASE_MODELS.TEST_THREES} (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${TEST_DATABASE_MODELS.TEST_THREES} }
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.TEST_THREES} }
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const test_threes = dataResult.map((row: Record<string, unknown>) =>
   ...
   */
  async combobox(manager: EntityManager): Promise<TestThree[]> {
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.TEST_THREES} }
      WHERE deleted_at IS NULL
  ...
  private entityToModel(entity: Record<string, unknown>): TestThree {
    return TestThree.fromPersistence(entity);
  }
}
