...
@Injectable()
export class TestOneRepositoryImpl implements TestOneRepository<EntityManager> {
  async create(test_one: TestOne, manager: EntityManager): Promise<TestOne> {
    const query = `
    INSERT INTO ${TEST_DATABASE_MODELS.TEST_ONES} (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${TEST_DATABASE_MODELS.TEST_ONES} }
   ...
   */
  async findById(id: number, manager: EntityManager): Promise<TestOne | null> {
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.TEST_ONES} }
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const test_ones = dataResult.map((row: Record<string, unknown>) =>
   ...
   */
  async combobox(manager: EntityManager): Promise<TestOne[]> {
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.TEST_ONES} }
      WHERE deleted_at IS NULL
  ...
  private entityToModel(entity: Record<string, unknown>): TestOne {
    return TestOne.fromPersistence(entity);
  }
}
