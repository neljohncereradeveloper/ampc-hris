...
export class EducationSchoolRepositoryImpl implements EducationSchoolRepository<EntityManager> {
  async create(
    ...
    const query = `
    INSERT INTO ${TEST_DATABASE_MODELS.EDUCATION_SCHOOLS} (
      desc1, created_by
    )
    VALUES ($1, $2)
    RETURNING *
  ...
  async update(
    ...
    const query = `
      UPDATE ${TEST_DATABASE_MODELS.EDUCATION_SCHOOLS} }
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${TEST_DATABASE_MODELS.EDUCATION_SCHOOLS} }
      WHERE id = $1
    `;
  ...
  private entityToModel(entity: Record<string, unknown>): EducationSchool {
    return EducationSchool.fromPersistence(entity);
  }
}
