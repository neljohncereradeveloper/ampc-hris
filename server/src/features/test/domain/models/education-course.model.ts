...
import { EducationCourseBusinessException } from '../exceptions/education-course.exception';
import { toLowerCaseString, toNumber } from '@/core/utils/coercion.util';

/**
 * EducationCourse domain entity.
 *
 * Encapsulates all business rules and state transitions for a educationCourse.
 *
 * Two ways to instantiate:
 * - `EducationCourse.create()` — for new educationCourse (validates business rules)
 * - `EducationCourse.fromPersistence()` — for rehydrating from the database (no validation)
 */
export class EducationCourse {
  /** Auto-incremented primary key. Null when not yet persisted. */
  id?: number | null;

  /** educationCourse description / name. e.g. 'manager', 'supervisor', etc. */
  desc1: string;

  /** Who created this educationCourse. Required at creation time. */
  created_by: string;

  /**
   * Timestamp when this educationCourse was created.
  ...
  /** Who last updated this educationCourse. Null until first update. */
  ...
  /** Who archived (soft-deleted) this educationCourse. Null if not archived. */
  deleted_by: string | null;

  /** Timestamp when this educationCourse was archived. Null if not archived. */
  deleted_at: Date | null;

  /**
   * Normalizes and initializes educationCourse fields from a raw DTO.
   *
   * Responsibilities:
  ...
  /**
   * Static factory method — the preferred way to create a new educationCourse.
   *
   * Constructs and validates the educationCourse in one step.
   * Throws `EducationCourseBusinessException` if any business rule is violated.
   */
  static create(params: {
    desc1: string;
    created_by: string;
  }): EducationCourse {
    const education_course = new EducationCourse({
      desc1: params.desc1,
      created_by: params.created_by,
    });
    education_course.validate();
    return education_course;
  }

  /**
   * Rehydrates a educationCourse from a raw database record.
   *
   ...
   */
  static fromPersistence(entity: Record<string, unknown>): EducationCourse {
  ...
  /**
   * Updates the educationCourse description and audit fields.
   *
   * - Throws if the educationCourse is currently archived.
  ...
  update(dto: { desc1: string; updated_by?: string | null }): void {
    if (this.deleted_at) {
      throw new EducationCourseBusinessException(
        'EducationCourse is archived and cannot be updated.',
  ...
  /**
   * Soft-deletes the educationCourse (archive).
   *
   * - Throws if the educationCourse is already archived.
  ...
  archive(deleted_by: string): void {
    if (this.deleted_at) {
      throw new EducationCourseBusinessException(
        'EducationCourse is already archived.',
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  /**
   * Restores an archived educationCourse.
   *
   * - Throws if the educationCourse is not currently archived.
   * - Clears `deleted_at` and `deleted_by`.
   */
  restore(): void {
    if (!this.deleted_at) {
      throw new EducationCourseBusinessException(
        `EducationCourse with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
   ...
   * Throws `EducationCourseBusinessException` with BAD_REQUEST on any violation.
   */
  validate(): void {
    // desc1 validations
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new EducationCourseBusinessException(
        'EducationCourse description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new EducationCourseBusinessException(
        'EducationCourse description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new EducationCourseBusinessException(
        'EducationCourse description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
