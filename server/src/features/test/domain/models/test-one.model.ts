...
import { TestOneBusinessException } from '../exceptions/test-one.exception';
import { toLowerCaseString, toNumber } from '@/core/utils/coercion.util';

/**
 * TestOne domain entity.
 *
 * Encapsulates all business rules and state transitions for a testOne.
 *
 * Two ways to instantiate:
 * - `TestOne.create()` — for new testOne (validates business rules)
 * - `TestOne.fromPersistence()` — for rehydrating from the database (no validation)
 */
export class TestOne {
  /** Auto-incremented primary key. Null when not yet persisted. */
  id?: number | null;

  /** testOne description / name. e.g. 'manager', 'supervisor', etc. */
  desc1: string;

  /** Who created this testOne. Required at creation time. */
  created_by: string;

  /**
   * Timestamp when this testOne was created.
  ...
  /** Who last updated this testOne. Null until first update. */
  ...
  /** Who archived (soft-deleted) this testOne. Null if not archived. */
  deleted_by: string | null;

  /** Timestamp when this testOne was archived. Null if not archived. */
  deleted_at: Date | null;

  /**
   * Normalizes and initializes testOne fields from a raw DTO.
   *
   * Responsibilities:
  ...
  /**
   * Static factory method — the preferred way to create a new testOne.
   *
   * Constructs and validates the testOne in one step.
   * Throws `TestOneBusinessException` if any business rule is violated.
   */
  static create(params: { desc1: string; created_by: string }): TestOne {
    const test_one = new TestOne({
      desc1: params.desc1,
  ...
  /**
   * Rehydrates a testOne from a raw database record.
   *
   ...
   */
  static fromPersistence(entity: Record<string, unknown>): TestOne {
    return new TestOne({
  ...
  /**
   * Updates the testOne description and audit fields.
   *
   * - Throws if the testOne is currently archived.
  ...
  update(dto: { desc1: string; updated_by?: string | null }): void {
    if (this.deleted_at) {
      throw new TestOneBusinessException(
        'TestOne is archived and cannot be updated.',
  ...
  /**
   * Soft-deletes the testOne (archive).
   *
   * - Throws if the testOne is already archived.
  ...
  /**
   * Restores an archived testOne.
   *
   * - Throws if the testOne is not currently archived.
   * - Clears `deleted_at` and `deleted_by`.
   */
  restore(): void {
    if (!this.deleted_at) {
      throw new TestOneBusinessException(
        `TestOne with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
   ...
   * Throws `TestOneBusinessException` with BAD_REQUEST on any violation.
   */
  validate(): void {
    // desc1 validations
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new TestOneBusinessException(
        'TestOne description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new TestOneBusinessException(
        'TestOne description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new TestOneBusinessException(
        'TestOne description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
