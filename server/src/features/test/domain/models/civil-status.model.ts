...
import { CivilStatusBusinessException } from '../exceptions/civil-status.exception';
import { toLowerCaseString, toNumber } from '@/core/utils/coercion.util';

/**
 * CivilStatus domain entity.
 *
 * Encapsulates all business rules and state transitions for a civilStatus.
 *
 * Two ways to instantiate:
 * - `CivilStatus.create()` — for new civilStatus (validates business rules)
 * - `CivilStatus.fromPersistence()` — for rehydrating from the database (no validation)
 */
export class CivilStatus {
  /** Auto-incremented primary key. Null when not yet persisted. */
  id?: number | null;

  /** civilStatus description / name. e.g. 'manager', 'supervisor', etc. */
  desc1: string;

  /** Who created this civilStatus. Required at creation time. */
  created_by: string;

  /**
   * Timestamp when this civilStatus was created.
  ...
  /** Who last updated this civilStatus. Null until first update. */
  ...
  /** Who archived (soft-deleted) this civilStatus. Null if not archived. */
  deleted_by: string | null;

  /** Timestamp when this civilStatus was archived. Null if not archived. */
  deleted_at: Date | null;

  /**
   * Normalizes and initializes civilStatus fields from a raw DTO.
   *
   * Responsibilities:
  ...
  /**
   * Static factory method — the preferred way to create a new civilStatus.
   *
   * Constructs and validates the civilStatus in one step.
   * Throws `CivilStatusBusinessException` if any business rule is violated.
   */
  static create(params: { desc1: string; created_by: string }): CivilStatus {
    const civil_status = new CivilStatus({
      desc1: params.desc1,
  ...
  /**
   * Rehydrates a civilStatus from a raw database record.
   *
   ...
   */
  static fromPersistence(entity: Record<string, unknown>): CivilStatus {
  ...
  /**
    * Updates the civilStatus description and audit fields.
   *
   * - Throws if the civilStatus is currently archived.
  ...
  update(dto: { desc1: string; updated_by?: string | null }): void {
    if (this.deleted_at) {
      throw new CivilStatusBusinessException(
        'CivilStatus is archived and cannot be updated.',
  ...
  /**
   * Soft-deletes the civilStatus (archive).
   *
   * - Throws if the civilStatus is already archived.
  ...
  archive(deleted_by: string): void {
    if (this.deleted_at) {
      throw new CivilStatusBusinessException(
        'CivilStatus is already archived.',
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  /**
   * Restores an archived civilStatus.
   *
   * - Throws if the civilStatus is not currently archived.
   * - Clears `deleted_at` and `deleted_by`.
   */
  restore(): void {
    if (!this.deleted_at) {
      throw new CivilStatusBusinessException(
        `CivilStatus with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
   ...
   * Throws `CivilStatusBusinessException` with BAD_REQUEST on any violation.
   */
  validate(): void {
    // desc1 validations
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new CivilStatusBusinessException(
        'CivilStatus description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new CivilStatusBusinessException(
        'CivilStatus description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new CivilStatusBusinessException(
        'CivilStatus description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
