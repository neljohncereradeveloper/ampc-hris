...
import { TestTwoBusinessException } from '../exceptions/test-two.exception';
import { toLowerCaseString, toNumber } from '@/core/utils/coercion.util';

/**
 * TestTwo domain entity.
 *
 * Encapsulates all business rules and state transitions for a testTwo.
 *
 * Two ways to instantiate:
 * - `TestTwo.create()` — for new testTwo (validates business rules)
 * - `TestTwo.fromPersistence()` — for rehydrating from the database (no validation)
 */
export class TestTwo {
  /** Auto-incremented primary key. Null when not yet persisted. */
  id?: number | null;

  /** testTwo description / name. e.g. 'manager', 'supervisor', etc. */
  desc1: string;

  /** Who created this testTwo. Required at creation time. */
  created_by: string;

  /**
   * Timestamp when this testTwo was created.
  ...
  /** Who last updated this testTwo. Null until first update. */
  ...
  /** Who archived (soft-deleted) this testTwo. Null if not archived. */
  deleted_by: string | null;

  /** Timestamp when this testTwo was archived. Null if not archived. */
  deleted_at: Date | null;

  /**
   * Normalizes and initializes testTwo fields from a raw DTO.
   *
   * Responsibilities:
  ...
  /**
   * Static factory method — the preferred way to create a new testTwo.
   *
   * Constructs and validates the testTwo in one step.
   * Throws `TestTwoBusinessException` if any business rule is violated.
   */
  static create(params: { desc1: string; created_by: string }): TestTwo {
    const test_two = new TestTwo({
      desc1: params.desc1,
  ...
  /**
   * Rehydrates a testTwo from a raw database record.
   *
   ...
   */
  static fromPersistence(entity: Record<string, unknown>): TestTwo {
    return new TestTwo({
  ...
  /**
   * Updates the testTwo description and audit fields.
   *
   * - Throws if the testTwo is currently archived.
  ...
  update(dto: { desc1: string; updated_by?: string | null }): void {
    if (this.deleted_at) {
      throw new TestTwoBusinessException(
        'TestTwo is archived and cannot be updated.',
  ...
  /**
   * Soft-deletes the testTwo (archive).
   *
   * - Throws if the testTwo is already archived.
  ...
  /**
   * Restores an archived testTwo.
   *
   * - Throws if the testTwo is not currently archived.
   * - Clears `deleted_at` and `deleted_by`.
   */
  restore(): void {
    if (!this.deleted_at) {
      throw new TestTwoBusinessException(
        `TestTwo with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
   ...
   * Throws `TestTwoBusinessException` with BAD_REQUEST on any violation.
   */
  validate(): void {
    // desc1 validations
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new TestTwoBusinessException(
        'TestTwo description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new TestTwoBusinessException(
        'TestTwo description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new TestTwoBusinessException(
        'TestTwo description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
