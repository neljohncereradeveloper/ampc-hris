...
@Injectable()
export class ArchiveJobtitleUseCase {
  constructor(
  ...
  async execute(id: number, requestInfo?: RequestInfo): Promise<boolean> {
    return this.transactionHelper.executeTransaction(
      ...
      async (manager) => {
        ...
        jobtitle.archive(requestInfo?.user_name || '');
        ...
        if (!success) {
          throw new JobtitleBusinessException(
            'Jobtitle archive failed',
            HTTP_STATUS.INTERNAL_SERVER_ERROR,
          );
        }

        const log = ActivityLog.create({
          action: JOBTITLE_ACTIONS.ARCHIVE,
          entity: SHARED_DOMAIN_DATABASE_MODELS.JOBTITLES,
          details: JSON.stringify({
            id,
            desc1: jobtitle.desc1,
            explanation: `Jobtitle with ID : ${id} archived by USER : ${requestInfo?.user_name || ''}`,
            archived_by: requestInfo?.user_name || '',
            archived_at: getPHDateTime(jobtitle.deleted_at || new Date()),
          }),
