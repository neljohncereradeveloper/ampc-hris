...
export class EducationCourseLevelRepositoryImpl implements EducationCourseLevelRepository<EntityManager> {
  async create(
    ...
    const query = `
      INSERT INTO ${MANAGEMENT_201_DATABASE_MODELS.EDUCATION_COURSE_LEVELS} (
    ...
    const result = await manager.query(query, [
      education_course_level.desc1,
      education_course_level.deleted_by,
      education_course_level.deleted_at,
      education_course_level.created_by,
      education_course_level.created_at,
      education_course_level.updated_by,
      education_course_level.updated_at,
    ]);
  ...
  async update(
    ...
    const query = `
      UPDATE ${MANAGEMENT_201_DATABASE_MODELS.EDUCATION_COURSE_LEVELS}
  ...
  async findPaginatedList(
    ...
    const education_course_levels = dataResult.map(
      (row: Record<string, unknown>) => this.entityToModel(row),
    );

    return {
      data: education_course_levels,
