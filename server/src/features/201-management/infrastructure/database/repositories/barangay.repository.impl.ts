...
@Injectable()
export class BarangayRepositoryImpl implements BarangayRepository<EntityManager> {
  async create(barangay: Barangay, manager: EntityManager): Promise<Barangay> {
    const query = `
      INSERT INTO ${MANAGEMENT_201_DATABASE_MODELS.BARANGAYS} (
  ...
  async update(
    ...
    const query = `
      UPDATE ${MANAGEMENT_201_DATABASE_MODELS.BARANGAYS}
  ...
  async findById(id: number, manager: EntityManager): Promise<Barangay | null> {
    const query = `
      SELECT *
      FROM ${MANAGEMENT_201_DATABASE_MODELS.BARANGAYS}
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const barangays = dataResult.map((row: Record<string, unknown>) =>
  ...
  async combobox(manager: EntityManager): Promise<Barangay[]> {
    const query = `
      SELECT id, desc1
      FROM ${MANAGEMENT_201_DATABASE_MODELS.BARANGAYS}
   ...
   */
  private entityToModel(entity: Record<string, unknown>): Barangay {
    return new Barangay({
