...
export class CivilStatusRepositoryImpl
  ...
  async create(
    ...
    const query = `
      INSERT INTO ${MANAGEMENT_201_DATABASE_MODELS.CIVIL_STATUSES} (
    ...
    const result = await manager.query(query, [
      civil_status.desc1,
      civil_status.deleted_by,
      civil_status.deleted_at,
      civil_status.created_by,
      civil_status.created_at,
      civil_status.updated_by,
  ...
  async update(
    ...
    const query = `
      UPDATE ${MANAGEMENT_201_DATABASE_MODELS.CIVIL_STATUSES}
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${MANAGEMENT_201_DATABASE_MODELS.CIVIL_STATUSES}
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const civil_statuses = dataResult.map((row: Record<string, unknown>) =>
