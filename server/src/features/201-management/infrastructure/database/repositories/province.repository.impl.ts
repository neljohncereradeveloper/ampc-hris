...
export class ProvinceRepositoryImpl implements ProvinceRepository<EntityManager> {
  ...
  async update(
    ...
    const query = `
      UPDATE ${MANAGEMENT_201_DATABASE_MODELS.PROVINCES}
  ...
  async findPaginatedList(
    ...
    const provinces = dataResult.map((row: Record<string, unknown>) =>
  ...
  async combobox(manager: EntityManager): Promise<Province[]> {
    const query = `
      SELECT id, desc1
      FROM ${MANAGEMENT_201_DATABASE_MODELS.PROVINCES}
  ...
  private entityToModel(entity: Record<string, unknown>): Province {
    return new Province({
