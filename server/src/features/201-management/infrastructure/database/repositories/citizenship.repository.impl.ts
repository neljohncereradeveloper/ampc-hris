...
export class CitizenshipRepositoryImpl
  ...
  async create(
    ...
    const query = `
      INSERT INTO ${MANAGEMENT_201_DATABASE_MODELS.CITIZENSHIPS} (
    ...
    const result = await manager.query(query, [
      citizenship.desc1,
      citizenship.deleted_by,
      citizenship.deleted_at,
      citizenship.created_by,
      citizenship.created_at,
      citizenship.updated_by,
      citizenship.updated_at,
    ]);
  ...
  async update(
    ...
    const query = `
      UPDATE ${MANAGEMENT_201_DATABASE_MODELS.CITIZENSHIPS}
  ...
  async findById(
    ...
    const query = `
      SELECT *
      FROM ${MANAGEMENT_201_DATABASE_MODELS.CITIZENSHIPS}
      WHERE id = $1
    `;
  ...
  async findPaginatedList(
    ...
    const citizenships = dataResult.map((row: Record<string, unknown>) =>
  ...
  async combobox(manager: EntityManager): Promise<Citizenship[]> {
    const query = `
      SELECT id, desc1
      FROM ${MANAGEMENT_201_DATABASE_MODELS.CITIZENSHIPS}
  ...
  private entityToModel(entity: Record<string, unknown>): Citizenship {
