...
@Injectable()
export class RestoreWorkExperienceCompanyUseCase {
  constructor(
  ...
  async execute(id: number, requestInfo?: RequestInfo): Promise<boolean> {
    return this.transactionHelper.executeTransaction(
      WORK_EXPERIENCE_COMPANY_ACTIONS.RESTORE,
      async (manager) => {
        // Validate existence
        ...
        work_experience_company.restore();
        ...
        if (!success) {
          throw new WorkExperienceCompanyBusinessException(
            'Work experience company restore failed',
        ...
        const log = ActivityLog.create({
          action: WORK_EXPERIENCE_COMPANY_ACTIONS.RESTORE,
          entity: MANAGEMENT_201_DATABASE_MODELS.WORK_EXPERIENCE_COMPANIES,
          details: JSON.stringify({
            id,
            desc1: work_experience_company.desc1,
            explanation: `Work experience company with ID : ${id} restored by USER : ${requestInfo?.user_name || ''
              }`,
