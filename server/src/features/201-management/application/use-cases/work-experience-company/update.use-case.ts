...
import { UpdateWorkExperienceCompanyCommand } from '../../commands/work-experience-company/update-work-experience-company.command';
import {
  getChangedFields,
  extractEntityState,
...
@Injectable()
export class UpdateWorkExperienceCompanyUseCase {
  constructor(
  ...
  async execute(
    id: number,
    command: UpdateWorkExperienceCompanyCommand,
    requestInfo?: RequestInfo,
  ): Promise<WorkExperienceCompany | null> {
    return this.transactionHelper.executeTransaction(
      WORK_EXPERIENCE_COMPANY_ACTIONS.UPDATE,
      async (manager) => {
        // Validate work experience company existence
        ...
        if (!work_experience_company) {
          throw new WorkExperienceCompanyBusinessException(
            'Work experience company not found',
        ...
        work_experience_company.update({
          desc1: command.desc1,
          updated_by: requestInfo?.user_name || null,
        });

        // Update the work experience company in the database
        ...
        if (!success) {
          throw new WorkExperienceCompanyBusinessException(
            'Work experience company update failed',
            HTTP_STATUS.INTERNAL_SERVER_ERROR,
          );
        }

        // Retrieve the updated work experience company
        ...
        const log = ActivityLog.create({
          action: WORK_EXPERIENCE_COMPANY_ACTIONS.UPDATE,
