...
@Injectable()
export class RestoreWorkExperienceJobTitleUseCase {
  constructor(
  ...
  async execute(id: number, requestInfo?: RequestInfo): Promise<boolean> {
    return this.transactionHelper.executeTransaction(
      WORK_EXPERIENCE_JOBTITLE_ACTIONS.RESTORE,
      async (manager) => {
        // Validate existence
        ...
        work_experience_job_title.restore();
        ...
        if (!success) {
          throw new WorkExperienceJobTitleBusinessException(
            'Work experience job title restore failed',
        ...
        const log = ActivityLog.create({
          action: WORK_EXPERIENCE_JOBTITLE_ACTIONS.RESTORE,
