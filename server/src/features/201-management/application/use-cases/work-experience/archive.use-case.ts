...
@Injectable()
export class ArchiveWorkExperienceUseCase {
  constructor(
  ...
  async execute(id: number, requestInfo?: RequestInfo): Promise<boolean> {
    return this.transactionHelper.executeTransaction(
      WORK_EXPERIENCE_ACTIONS.ARCHIVE,
      async (manager) => {
        ...
        work_experience.archive(requestInfo?.user_name || '');
        ...
        if (!success) {
          throw new WorkExperienceBusinessException(
            'Work experience archive failed',
            HTTP_STATUS.INTERNAL_SERVER_ERROR,
          );
        }

        const log = ActivityLog.create({
          action: WORK_EXPERIENCE_ACTIONS.ARCHIVE,
          ...
          details: JSON.stringify({
            ...
            explanation: `Work experience with ID : ${id} archived by USER : ${requestInfo?.user_name || ''
              }`,
            archived_by: requestInfo?.user_name || '',
            archived_at: getPHDateTime(
              work_experience.deleted_at || new Date(),
            ),
          }),
