...
import { EducationSchoolBusinessException } from '../exceptions';
...
export class EducationSchool {
  ...
  update(dto: {
    ...
    if (this.deleted_at) {
      throw new EducationSchoolBusinessException(
        'Education school is archived and cannot be updated',
        HTTP_STATUS.CONFLICT,
      );
    }
    const temp_education_school = new EducationSchool({
      id: this.id,
      desc1: dto.desc1,
      created_at: this.created_at,
      updated_at: this.updated_at,
    });
    temp_education_school.validate();
    this.desc1 = dto.desc1;
    this.updated_by = dto.updated_by ?? null;
  }
  ...
  archive(deleted_by: string): void {
    if (this.deleted_at) {
      throw new EducationSchoolBusinessException(
        'Education school is already archived.',
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  restore(): void {
    if (!this.deleted_at) {
      throw new EducationSchoolBusinessException(
        `Education school with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  validate(): void {
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new EducationSchoolBusinessException(
        'Education school name is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new EducationSchoolBusinessException(
        'Education school name must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 3) {
      throw new EducationSchoolBusinessException(
        'Education school name must be at least 3 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
