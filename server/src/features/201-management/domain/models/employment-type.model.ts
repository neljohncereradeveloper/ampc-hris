...
import { EmploymentTypeBusinessException } from '../exceptions';
...
export class EmploymentType {
  ...
  static create(params: {
    ...
    const employment_type = new EmploymentType({
      desc1: params.desc1,
      created_by: params.created_by ?? null,
    });
    employment_type.validate();
  ...
  update(dto: {
    ...
    if (this.deleted_at) {
      throw new EmploymentTypeBusinessException(
        'Employment type is archived and cannot be updated',
        HTTP_STATUS.CONFLICT,
      );
    }
    const temp_employment_type = new EmploymentType({
      id: this.id,
      desc1: dto.desc1,
      created_at: this.created_at,
      updated_at: this.updated_at,
    });
    temp_employment_type.validate();
    this.desc1 = dto.desc1;
    this.updated_by = dto.updated_by ?? null;
  }
  ...
  archive(deleted_by: string): void {
    if (this.deleted_at) {
      throw new EmploymentTypeBusinessException(
        'Employment type is already archived.',
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  restore(): void {
    if (!this.deleted_at) {
      throw new EmploymentTypeBusinessException(
        `Employment type with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  validate(): void {
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new EmploymentTypeBusinessException(
        'Employment type description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new EmploymentTypeBusinessException(
        'Employment type description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new EmploymentTypeBusinessException(
        'Employment type description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
