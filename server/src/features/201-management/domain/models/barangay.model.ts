...
import { BarangayBusinessException } from '../exceptions';
...
export class Barangay {
  ...
  update(dto: {
    ...
    const temp_barangay = new Barangay({
      id: this.id,
      desc1: dto.desc1,
      created_at: this.created_at,
      updated_at: this.updated_at,
    });
    temp_barangay.validate();
    this.desc1 = dto.desc1;
    this.updated_by = dto.updated_by ?? null;
  }
  ...
  validate(): void {
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new BarangayBusinessException(
        'Barangay description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new BarangayBusinessException(
        'Barangay description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new BarangayBusinessException(
        'Barangay description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
