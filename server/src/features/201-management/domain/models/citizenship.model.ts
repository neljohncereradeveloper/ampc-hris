...
import { CitizenshipBusinessException } from '../exceptions';
...
export class Citizenship {
  ...
  update(dto: {
    ...
    const temp_citizenship = new Citizenship({
      id: this.id,
      desc1: dto.desc1,
      created_at: this.created_at,
      updated_at: this.updated_at,
    });
    temp_citizenship.validate();
    this.desc1 = dto.desc1;
    this.updated_by = dto.updated_by ?? null;
  }
  ...
  archive(deleted_by: string): void {
    if (this.deleted_at) {
      throw new CitizenshipBusinessException(
        'Citizenship is already archived.',
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  restore(): void {
    if (!this.deleted_at) {
      throw new CitizenshipBusinessException(
        `Citizenship with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  validate(): void {
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new CitizenshipBusinessException(
        'Citizenship description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new CitizenshipBusinessException(
        'Citizenship description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new CitizenshipBusinessException(
        'Citizenship description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
