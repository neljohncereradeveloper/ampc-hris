...
import { ProvinceBusinessException } from '../exceptions';
...
export class Province {
  ...
  update(dto: { desc1: string; updated_by?: string | null }): void {
    if (this.deleted_at) {
      throw new ProvinceBusinessException(
        'Province is archived and cannot be updated',
        HTTP_STATUS.CONFLICT,
      );
    }
    const temp_province = new Province({
      id: this.id,
      desc1: dto.desc1,
      created_at: this.created_at,
      updated_at: this.updated_at,
    });
    temp_province.validate();
    this.desc1 = dto.desc1;
    this.updated_by = dto.updated_by ?? null;
  }
  ...
  restore(): void {
    if (!this.deleted_at) {
      throw new ProvinceBusinessException(
        `Province with ID ${this.id} is not archived.`,
        HTTP_STATUS.CONFLICT,
      );
    }
  ...
  validate(): void {
    if (!this.desc1 || this.desc1.trim().length === 0) {
      throw new ProvinceBusinessException(
        'Province description (desc1) is required and cannot be empty.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.length > 255) {
      throw new ProvinceBusinessException(
        'Province description (desc1) must not exceed 255 characters.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
    if (this.desc1.trim().length < 2) {
      throw new ProvinceBusinessException(
        'Province description (desc1) must be at least 2 characters long.',
        HTTP_STATUS.BAD_REQUEST,
      );
    }
  }
